	...
	try {
		...
		//poll device code	
			...
			do {
				...
				if (dr.respcode == 200) {
					...
					throw std::runtime_error("Auth device responce code: " + std::to_string(dr.respcode) + json::serialize(dr.resp));
				}	
		...
		//reauth 
			...
			if (current_time > expire_time) {
				...
				if (!(rr.respcode == 200)) throw std::runtime_error("reauth failed!" + std::to_string(rr.respcode) + json::serialize(rr.resp));
