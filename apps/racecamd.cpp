...
//    struct timeval press_time;
	...
	if (!RunProgram.load()) return; // SIGINT and SIGTERM stops read() wait so return
	...
	if (bytes_read == sizeof(ev)) {
            ...
            if (ev.type == EV_KEY) {
		if (ev.code < 1 || ev.code > 3) {
		    std::cerr << "button code invalid: " << ev.code << std::endl;
		    continue;
		} else if (ev.value == 1) {
	//	    press_time = ev.time;
		    continue;
...
int main(int argc, char *argv[])
{
    ...
    const char* dev = "/dev/input/by-path/platform-racecam_btns-event"; 
    int fd = open(dev, O_RDONLY);
    if (fd == -1) {
        std::perror("Cannot open device");
        std::cerr << "Make sure the device name is correct." << std::endl;
        return EXIT_FAILURE;
    }
